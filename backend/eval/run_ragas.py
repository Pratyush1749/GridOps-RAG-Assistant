...
from eval.invokers import ApiInvoker, ServiceInvoker, SkippedIntent
...
def main() -> None:
    ...
    invoker: ServiceInvoker | ApiInvoker
    if args.mode == "service":
        invoker = ServiceInvoker()
    else:
        # Drives the real HTTP API, so sql/hybrid goldens (which need the
        # LangGraph HITL approval flow) are actually scored.
        invoker = ApiInvoker()
    ...
    for row, m in zip(rows, metrics, strict=False):
        row["ragas_metrics"] = m
