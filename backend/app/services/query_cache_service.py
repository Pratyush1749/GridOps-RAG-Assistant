...
class QueryCacheService:
    ...
    def __init__(self) -> None:
        ...
        self._stats: dict[str, dict[str, int]] = {tier: defaultdict(int) for tier in self._TIERS}
    ...
    def rag_answer_key(self, question: str, cache_context: dict[str, Any] | None = None) -> str:
        ...
        return self._key(
            "rag_answer", json.dumps(raw, sort_keys=True) if isinstance(raw, dict) else raw
        )
    ...
    def embedding_key(self, text: str, model: str) -> str:
        # Namespaced by model/backend so a model change (or an OpenAI key
        # going from broken -> working, which silently swaps the backend
        # between the local fallback and OpenAI) can never serve a vector
        # produced by a different embedding model for the same text.
    ...
    def clear(self) -> list[str]:
        ...
        # Clear Redis — SCAN each namespace prefix and delete in batches.
        if self._redis_client is not None:
            deleted = 0
            try:
                for prefix in ("intent", "rag_answer", "sql_gen", "sql_result:v2", "embedding"):
                    ...
                    while True:
                        ...
                        if keys:
                            ...
                            deleted += len(keys)
                        # Upstash returns cursor "0" (or 0) when the scan is complete.
                        if str(cursor) == "0":
                            break
                cleared.append(f"redis ({deleted} keys)")
            except Exception:
                logger.exception("Redis clear failed")
                cleared.append("redis (failed - entries will expire via TTL)")
