...
def _flag(flags: dict | int | None, key: str, default):
...
def _generate(
    ...
    def _repair(prompt: str, _error: str) -> str:
        return generate(
            "You produce ONLY valid JSON matching the requested schema. "
            "No prose, no markdown fences, no commentary.",
            prompt,
        )["text"]

    def _clean_answer(raw_text: str) -> str:
        """Extract plain answer text from the LLM's JSON envelope.

        `build_system_prompt()` instructs the model to reply with
        `{"answer": ..., "sources": ..., "confidence": ...}`, but nothing
        parsed that back out before — the raw JSON string was stored (and
        shown to users / scored by RAGAS) as-is. Falls back to the raw text
        if parsing/repair ultimately fails, so a malformed response never
        breaks the request.
        """
        try:
            return validate_with_retry(raw_text, _repair).answer
        except Exception:
            logger.warning("Could not parse LLM JSON envelope; using raw text as answer")
            return raw_text

    working_q = question
    raw = _raw(working_q)
    answer_text = _clean_answer(raw)
...
def _run_hybrid_inline(
    ...
    spotlighted = build_spotlighted_context(chunks)
    system = (
        "You are a Grid Operations assistant. Synthesize the database query results "
