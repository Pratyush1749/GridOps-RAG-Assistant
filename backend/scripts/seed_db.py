...
# Prefer an explicit shell env var, otherwise use the same source the app does
# (app.config.settings, which loads .env / ../.env) so seeding and serving can
...
def seed_docs(noise_sample_size: int | str = 150) -> dict:
    ...
    for idx, src in enumerate(true_files, start=1):
        _ingest_one(
            processor, src, idx, total, counters, embed_texts, upsert_chunks, RetrievedChunk
        )
