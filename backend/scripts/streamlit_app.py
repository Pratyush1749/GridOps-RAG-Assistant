...
GridOps edition — per-lesson feature detection + Eval Dashboard.
"""
...
# All application routes are mounted under this prefix by app.main.create_app().
# `_request()` prepends it, so callers pass bare paths like "/query".
API_PREFIX = "/api/v1"
# FastAPI is configured with openapi_url="/api/openapi.json" (not the default).
OPENAPI_PATH = "/api/openapi.json"

USE_CASES: dict[str, dict[str, Any]] = {
    "🔌 Substation Overview": {
        "question": "What is a substation and what equipment does it contain?",
        "mode": "rag",
        "search_mode": "dense",
        "enable_hyde": False,
        "enable_rerank": False,
        ...
        "top_k": 5,
        "description": "L1 — Baseline dense search on grid-ops concepts",
    },
    "📝 Sparse Token (differential protection)": {
        "question": "What does a differential protection trip alarm indicate?",
        "mode": "rag",
        "search_mode": "sparse",
        "enable_hyde": False,
        "enable_rerank": False,
        ...
        "enable_self_reflective": False,
        "top_k": 5,
        "description": "L2 — Sparse BM25 for exact alarm-type phrases",
    },
    "⚡ Hybrid (transformer capacity)": {
        "question": "What loading threshold and alarm pattern should trigger a transformer capacity upgrade review?",
        "mode": "rag",
        "search_mode": "hybrid",
        "enable_hyde": False,
        "enable_rerank": False,
    ...
    "🎯 Rerank (SCADA severities)": {
        "question": "What are the SCADA alarm severity levels and their acknowledgement time targets?",
        "mode": "rag",
        "search_mode": "hybrid",
        "enable_hyde": False,
        "enable_rerank": True,
    ...
    "🧠 HyDE (paraphrased)": {
        "question": "How can I tell if a neighborhood's power loss will come back on its own without a crew being sent out?",
        "mode": "rag",
        "search_mode": "hybrid",
        "enable_hyde": True,
        "enable_rerank": True,
    ...
    "🌐 CRAG (FERC order — out-of-corpus)": {
        "question": "What is the latest FERC order on regional transmission planning cost allocation?",
        "mode": "rag",
        "search_mode": "hybrid",
        "enable_hyde": False,
        "enable_rerank": True,
    ...
    "🔄 Self-RAG (vague query)": {
        "question": "tell me about outages",
        "mode": "rag",
        "search_mode": "hybrid",
        "enable_hyde": False,
        "enable_rerank": True,
    ...
    "🗄️ Outage Count (SQL)": {
        "question": "How many P1 outages occurred in the last 30 days?",
        "mode": "sql",
        "search_mode": "hybrid",
        "enable_hyde": False,
        "enable_rerank": True,
        ...
        "enable_self_reflective": False,
        "top_k": 3,
        "description": "L7 — Text2SQL auto-routes to the grid_ops DB",
    },
    "📊 Crew Response Time (SQL)": {
        "question": "What is the average crew response time for outage dispatch?",
        "mode": "sql",
        "search_mode": "hybrid",
        "enable_hyde": False,
        "enable_rerank": False,
        ...
        "enable_self_reflective": False,
        "top_k": 3,
        "description": "L7 — Aggregation across crew_dispatch_logs",
    },
    "⚡ Cache Demo (Substation — run twice)": {
        "question": "What is a substation and what equipment does it contain?",
        "mode": "rag",
        "search_mode": "dense",
        "enable_hyde": False,
    ...
    "🚨 Storm event dispatch model": {
        "question": "How does crew dispatch priority change during a declared storm event?",
        "mode": "rag",
        "search_mode": "hybrid",
        "enable_hyde": False,
        "enable_rerank": True,
        ...
        "enable_self_reflective": False,
        "top_k": 5,
        "description": "Quick reference — storm/emergency response dispatch model",
    },
    "🏆 Hybrid Outage + Dispatch (All Features)": {
        "question": (
            "How many P1 outages occurred in the last 30 days, and what is the "
            "recommended crew dispatch response for that severity?"
        ),
        "mode": "hybrid",
        "search_mode": "hybrid",
        "enable_hyde": True,
...
def detect_lesson_features(base_url: str) -> dict:
    ...
    """
    try:
        r = requests.get(f"{base_url.rstrip('/')}{OPENAPI_PATH}", timeout=5)
        ...
        if "lesson-" in version:
            ...
            lesson = "unknown"
        qr_schema = spec.get("components", {}).get("schemas", {}).get("QueryRequest", {})
        props: set[str] = set(qr_schema.get("properties", {}).keys()) if qr_schema else set()
        has_query = any(p.endswith("/query") for p in spec.get("paths", {}))
        return {
            "version": version,
            "lesson": lesson,
...
def _lesson_banner(info: dict) -> None:
    ...
    if lesson == "unknown":
        ...
        st.success(
            f"📚 **Current Lesson:** `{lesson}` · Features: `{features_str}`",
            icon="🎓",
        )
...
def _sidebar(base_url: str) -> str:
    with st.sidebar:
        st.image(
            "https://img.shields.io/badge/ADV--RAG-GridOps-009688?style=for-the-badge&logo=flashforge",
    ...
    if st.button("🚀 Upload & Index", use_container_width=True, key="btn_upload"):
        ...
        with st.status("Indexing document…", expanded=True) as status:
            ...
            try:
                resp = requests.request(
                    method="POST",
                    url=f"{base_url.rstrip('/')}{API_PREFIX}/documents/upload",
                    headers={"Authorization": f"Bearer {token}"},
                    files=files,
                    timeout=600,
                )
                ...
                st.error(
                    "Could not connect to the API. Is the server running at the configured base URL?"
                )
                return
            ...
            if status_code in (200, 201) and isinstance(payload, dict):
                ...
                st.write(
                    f"✅ **Step 2/3** — Parsed {chunks} chunk(s)"
                ...
                status.update(
                    label=f"❌ Upload failed (HTTP {status_code})", state="error", expanded=True
                )
...
def _query_section(base_url: str, lesson_info: dict) -> None:
    ...
    for row_start in range(0, len(preset_items), 4):
        ...
        for idx, (label, cfg) in enumerate(row):
            with cols[idx]:
                ...
                if st.button(
                    label, use_container_width=True, key=f"preset_{row_start + idx}", help=help_text
                ):
    ...
    with st.expander("⚙️ RAG Feature Toggles", expanded=True):
        ...
        st.caption(
            f"Active features: **{active_str}** | Search: **{search_mode}** | top_k: **{top_k}**"
        )
...
def _eval_dashboard_section() -> None:
    ...
    with f1:
        sel_features = st.multiselect(
            "Filter by feature", features, default=features, key="eval_filter_feature"
        )
    with f2:
        sel_intents = st.multiselect(
            "Filter by intent", intents, default=intents, key="eval_filter_intent"
        )
    with f3:
        sel_statuses = st.multiselect(
            "Filter by status", statuses, default=statuses, key="eval_filter_status"
        )
    with f4:
        ...
        table_rows.append(
            {
                ...
                "Prec": _delta_cell(
                    ragas.get("context_precision"), prev_ragas.get("context_precision")
                ),
                "Recall": _delta_cell(
                    ragas.get("context_recall"), prev_ragas.get("context_recall")
                ),
                "Ans Rel": _delta_cell(
                    ragas.get("answer_relevancy"), prev_ragas.get("answer_relevancy")
                ),
            }
        )
    ...
    sel_id = st.selectbox(
        ...
        format_func=lambda qid: (
            f"{qid} — "
            + ((next((r.get("question", "")[:80] for r in rows if r.get("id") == qid), "")) or "")
        ),
